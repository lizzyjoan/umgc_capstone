...
#brca_tpm <- dplyr::select(brca_tpm, matches(row.names(brca_drug_meta)))
```
...
Survivor subset and non-survivor subset
```{r}
alive_meta <- dplyr::filter(brca_drug_meta, tcga.gdc_cases.diagnoses.vital_status == "alive")

dead_meta <- dplyr::filter(brca_drug_meta, tcga.gdc_cases.diagnoses.vital_status == "dead")
```

Make new metadata, control vs tumor (can add in other features here to be used as covariates, i.e. age, race, etc.)
```{r}
lm_meta <- data.frame("Samples" = c(colnames(brca_drug_meta), colnames(ctl_test)), Condition = c(rep("Tumor", length(brca_test)), rep("Control", length(ctl_test))))
...
brca_drug_meta$tcga.gdc_cases.diagnoses.vital_status <- relevel(brca_drug_meta$tcga.gdc_cases.diagnoses.vital_status, 'alive') #reorder to make 'alive' the reference

des <- model.matrix(~brca_drug_meta$tcga.gdc_cases.diagnoses.vital_status)
```
