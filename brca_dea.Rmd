...
Read back in
```{r}
brca_primary_metadata <- read.csv("./data/brca_primarytumor_metadata.csv")
...
#                 values=gene_ref,
#                 mart= mart)
protCod = convert[which(gene_desc$gene_biotype == 'protein_coding'), ]
protCodIDs = protCod$ensembl_gene_id
#brca.omics = temp[, match(protCodIDs, temp$gene)]
...
#t <- dplyr::intersect(brca_scaled, brca_primary_metadata$tcga.cgc_case_vital_status)
brca_scaled$output <- brca_primary_metadata$tcga.cgc_case_vital_status #brca_primary_metadata[match(row.names(brca_scaled),row.names(brca_primary_metadata)),"tcga.cgc_case_vital_status"]
#brca_scaled <- brca_scaled %>% tidyr::drop_na(tcga.cgc_case_vital_status)
```
...
train <- caret::createDataPartition(y = brca_scaled$output, p = 0.8, list = FALSE, times = 1)
brca.data_train <- brca_scaled[train, ]
brca.data_test  <- brca_scaled[-train, ]
...
brca_cts_nzv <- brca_scaled[,-nzv]
```

if needed, can also come back to this spot to remove extra correlations (under 'preprocessing' https://cafernandezlo.github.io/es_fic_mubics_caret/caret.html#Caret )
