...
```{r}
write.csv(brca_metadata, file = "tcga_brca_metadata.csv")
...
#remove patients with missing vital data
brca_primary_metadata <- tidyr::drop_na(brca_primary_metadata, tcga.cgc_case_vital_status) #1117 samples
...
```{r}
library(magrittr)
#remove patients with missing drug data
brca_primary_metadata <- tidyr::drop_na(brca_primary_metadata, tcga.cgc_drug_therapy_drug_name) #763
brca_primary_metadata$tcga.cgc_drug_therapy_drug_name <- tolower(brca_primary_metadata$tcga.cgc_drug_therapy_drug_name)

brca_drugs_binned <- brca_primary_metadata$tcga.cgc_drug_therapy_drug_name

brca_drugs_binned <- sub("anastrozole|anastrozolum|anastrazole", "arimidex", brca_drugs_binned)
brca_drugs_binned <- sub("nolvadex", "tamoxifen", brca_drugs_binned)
brca_drugs_binned <- sub("paclitaxelum|taxol", "paclitaxel", brca_drugs_binned)
brca_drugs_binned <- sub("5 fluorouracil|5-fluorouracil|5-flourouracil", "fluorouracil", brca_drugs_binned)
brca_drugs_binned <- sub("doxorubicin hcl|doxorubicinum|doxorubicine|adriamycin|adriamycin|adriamyicin|adrimycin", "doxorubicin", brca_drugs_binned)
brca_drugs_binned <- sub("cyotxan|cyotxan|doxorubicine", "cytoxan", brca_drugs_binned)
brca_drugs_binned <- sub("aromatase exemestane|aromasin (exemestane)|aromasin", "exemestane", brca_drugs_binned)
brca_drugs_binned <- sub("letrozole (femara)|femara|letrozol", "letrozole", brca_drugs_binned)

#make new column in metadata
brca_primary_metadata$brca_drugs_binned <- brca_drugs_binned

#plot, comparing live vs dead
ggplot(data = brca_primary_metadata, aes(x = brca_drugs_binned, y = ..count.., fill = tcga.cgc_case_vital_status)) +
  geom_bar() +
...
Read back in
```{r}
brca_primary_metadata <- read.csv("./data/brca_primarytumor_metadata.csv", row.names = 1)

#brca_tpm <- read.csv("./data/brca_primarytumor_tpm_expdata.csv")
...
toptrts_var_pca_plot <- autoplot(toptrts_var_pca,
						data = meta_cts,
						colour = 'tcga.cgc_case_pathologic_stage',  x = 1, y = 2) 
...
GGally::ggpairs(data=brca_toptrt_meta, columns= c("tcga.gdc_cases.diagnoses.vital_status", "tcga.cgc_case_pathologic_stage"), title="Associations to Survival")
...
Make a design matrix, use gender and previous diagnosis as blocking factors
```{r}
brca_primary_metadata$tcga.gdc_cases.diagnoses.vital_status <- as.factor(brca_primary_metadata$tcga.gdc_cases.diagnoses.vital_status)
brca_primary_metadata$tcga.gdc_cases.diagnoses.vital_status <- relevel(brca_primary_metadata$tcga.gdc_cases.diagnoses.vital_status, 'alive') #reorder to make 'alive' the reference

des <- model.matrix(~brca_primary_metadata$tcga.gdc_cases.diagnoses.vital_status)
```

Filtering out genes with zero variance
```{r}
#converted gene id's for counts 
brca_tpm <- dplyr::select(brca_cts, matches(row.names(brca_primary_metadata)))

#makes object for convenient use
dge  <- edgeR::DGEList(counts=brca_tpm)
...
```{r}
#absolute log2fc > 2
#lmres  <- limma::treat(fit , fc = 0.5, trend=TRUE)
lmres <- limma::eBayes(fit)

#compute BH p-adjusted values, only values meeting 0.05 cutoff, avg expression, and all in nice table like top 10
#lmres  <- topTreat(lmres , coef=ncol(des), sort.by = "logFC", number = nrow(fit [["coefficients"]]), p.value = 0.05)
tfit <- treat(lmres, fc=1.1)
t <- topTreat(tfit,coef=2, number = 50)
t
...
```{r}
write.csv(t, file = "brca_survivors_degs.csv")
```

```{r}
library(EnhancedVolcano)
brca_degs_volcano <- EnhancedVolcano(t, lab = rownames(t), x = 'logFC', y = 'P.Value', pCutoff = 0.01, FCcutoff = 0.5, pointSize = 3.0, labSize = 4.0, col = c("#440154", "#21918c", "#fde725", "#5ec962"), title = "BRCA Survival DEGs", labFace = 'bold', boxedLabels = TRUE,drawConnectors = TRUE,widthConnectors = 0.5,colConnectors = 'black', max.overlaps = 70) 

brca_degs_volcano
```

```{r}
ggsave(brca_degs_volcano, filename = "volcano_brca_degs.png")
```

```{r}
brca_drug_meta$tcga.gdc_cases.diagnoses.vital_status <- as.factor(brca_drug_meta$tcga.gdc_cases.diagnoses.vital_status)
#print(rownames(metadata))
#print(colnames(cts))
library(DESeq2)
cts <- as.data.frame(round(assay(brca_data, "counts")))
cts <- dplyr::select(cts, matches(row.names(brca_drug_meta)))

cts <- dplyr::select(cts, matches(row.names(brca_drug_meta)))

dds <- DESeqDataSetFromMatrix(cts, brca_drug_meta, design = ~ tcga.gdc_cases.diagnoses.vital_status)
...
#log fold shrinkage to adjust for very lowly expressed genes that change just a little bit, helps to remove weird small changes in pseudogenes/lncRNAs/etc
#dds_lfc <- lfcShrink(dds_res, coef = c("Genotype_Pkd2flflPax8rtTATetOcre_vs_Wildtype"), type="apeglm")


#NAs removed first
#res <- res[!is.na(dds_lfc$padj),]

#p-adjusted value cutoff 0.05 and log2fc greater than 2.0 and less than -2.0 
upgenes <- res[res$log2FoldChange > 1.1 & res$padj < 0.05,]

```

### Gene conversion  
Adapted from https://cafernandezlo.github.io/es_fic_mubics_caret/caret.html#Caret 
```{r}
brca_tpm <- as.data.frame((brca_data@assays@data@listData$TPM))
temp <- tibble::rownames_to_column(brca_tpm, var = "gene")
temp$gene <- gsub("\\..*", "", temp$gene)
#gene_ref <- unique(gsub("\\..*", "", rownames(brca_tpm)))

mart <- useEnsembl("ensembl", dataset = "hsapiens_gene_ensembl", mirror = "useast")
gene_desc <- getBM(filters = "ensembl_gene_id", attributes = c("ensembl_gene_id","gene_biotype", "hgnc_symbol"), values = temp$gene, mart = mart)

#find all protein-coding genes
#protCod = gene_desc[which(gene_desc$gene_biotype == 'protein_coding'), ]
#protCodIDs = protCod$ensembl_gene_id

#filter for protein-coding genes
#brca_cts <- dplyr::filter(temp, gene %in% protCodIDs)

#merge count data and gene conversion table
brca_cts <- merge(brca_cts, gene_desc, by.x = "gene", by.y = "ensembl_gene_id")

#remove duplicate gene symbols
brca_cts <- dplyr::distinct(brca_cts, hgnc_symbol, .keep_all = TRUE)

#Convert back to rownames but with gene symbols
brca_cts <- tibble:::column_to_rownames(brca_cts, "hgnc_symbol")

#get rid of extra gene id columns
brca_cts <- subset(brca_cts, select=-c(gene, gene_biotype))

```

### Data processing
Scale TPM counts (should give a mean around 0 and std dev 1)
```{r}
brca_scaled <- as.data.frame(scale(t(brca_cts)))
```

combine scaled counts to vital status data
```{r}
brca_primary_metadata$tcga.cgc_case_vital_status <- as.factor(brca_primary_metadata$tcga.cgc_case_vital_status)

brca_scaled <- dplyr::filter(brca_scaled, row.names(brca_scaled) %in%  row.names(brca_primary_metadata) )

brca_scaled$output <- brca_primary_metadata$tcga.cgc_case_vital_status 

```

remove near zero variance features
```{r}
nzv <- nearZeroVar(brca_scaled)
brca_cts.nzv <- brca_scaled[,-nzv]
...
https://cafernandezlo.github.io/es_fic_mubics_caret/caret.html#Caret )

```{r}
output <- brca_cts.nzv %>% dplyr::select('output')
data <- brca_cts.nzv[,setdiff(colnames(brca_cts.nzv),'output')]
high.cor <- findCorrelation(data, cutoff = .75)

brca.data.nzv.cor <- brca_cts.nzv[,-high.cor]
...
#brca.data_train <- brca.data_train %>% #dplyr::select(names(brca.data.nzv.cor))
#brca.data_test <- brca.data_test %>% dplyr::select(names(brca.data.nzv.cor))
```
...
train <- caret::createDataPartition(y = brca.data.nzv.cor$output, p = 0.8, list = FALSE, times = 1)
brca.data_train <- brca.data.nzv.cor[train, ]
brca.data_test  <- brca.data.nzv.cor[-train, ]

#futher remove NAs??
#brca.data_train <- as.data.frame(t(brca.data_train))
#brca.data_train <- drop_na(brca.data_train)
#brca.data_train <- as.data.frame(t(brca.data_train))
...
set.seed(1234)
#seeds <- vector(mode = "list", length = (cv.k * reps) + 6)
#for (i in 1:(cv.k * reps)) {
#  seeds[[i]] <- sample.int(1000, nrow(hyperparameters))
#}
#seeds[[(cv.k * reps) + 6]] <- sample.int(1000, 1)

train.control <- trainControl(method = "repeatedcv", number = 5, repeats = reps, returnResamp = "final", verboseIter = TRUE, allowParallel = TRUE)
```

model
```{r}
set.seed(5678)
rf.model <- caret::train(x = brca.data_train[,-ncol(brca.data_train)],
...
most important genes in model
```{r}
importance.rf <- varImp(rf.model,scale = TRUE)
plot(importance.rf, main = 'RF (ranger). Gene importance',top=20)
...
preds.results <- extractPrediction(
                  models = results,
                  testX = brca.data_test %>% dplyr::select(-output),
...
preds.scores %>%
  tidyr::spread(key = dataType, value = accuracy) %>%
  arrange(desc(Test))
